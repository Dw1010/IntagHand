...
def get_mano_path(cfg):
    MISC = cfg.MISC
    path = os.path.join(MISC.CKPT_PATH, MISC.MANO_PATH)
    mano_path = {
        "left": os.path.join(path, "MANO_LEFT.pkl"),
        "right": os.path.join(path, "MANO_RIGHT.pkl"),
    }
    return mano_path


def get_graph_dict_path(cfg):
    MISC = cfg.MISC
    graph_path = {
        "left": os.path.join(MISC.CKPT_PATH, MISC.GRAPH_LEFT_DICT_PATH),
        "right": os.path.join(MISC.CKPT_PATH, MISC.GRAPH_RIGHT_DICT_PATH),
    }
    return graph_path


def get_dense_color_path(cfg):
    MISC = cfg.MISC
    dense_path = os.path.join(MISC.CKPT_PATH, MISC.DENSE_COLOR)
    return dense_path
...
def get_upsample_path(cfg):
    MISC = cfg.MISC
    upsample_path = os.path.join(MISC.CKPT_PATH, MISC.UPSAMPLE_PATH)
...
class imgUtils:
    ...
    @staticmethod
    def get_affine_mat(theta=0, scale=1.0, u=0, v=0, height=480, width=640):
    ...
    def img_trans(theta, scale, u, v, img):
        ...
        u = int(u * size / 2)
        v = int(v * size / 2)
        affineMat = imgUtils.get_affine_mat(
            theta=theta, scale=scale, u=u, v=v, height=256, width=256
        )
    ...
    def data_augmentation(
        ...
        affineMat = imgUtils.get_affine_mat(
            theta=theta, scale=scale, u=u, v=v, height=img_size, width=img_size
        )
        if img_list is not None:
            ...
            for img in img_list:
                img_list_out.append(
                    cv.warpAffine(
                        src=img, M=affineMat[0:2, :], dsize=(img_size, img_size)
                    )
                )
        else:
            img_list_out = None
        ...
        if label2d_list is not None:
            ...
            for label2d in label2d_list:
                label2d_list_out.append(
                    np.matmul(label2d, affineMat[0:2, 0:2].T) + affineMat[0:2, 2:3].T
                )
        else:
    ...
    def add_noise(img, noise=0.00, scale=255.0, alpha=0.3, beta=0.05):
        ...
        img = (
            a * img + b + scale * np.random.normal(loc=0.0, scale=noise, size=img.shape)
        )
