...
class InterRender:
    def __init__(
        ...
        self.renderer = mano_two_hands_renderer(
            cfg,
        ...
        MISC = cfg.MISC
        model_path = f"{MISC.CKPT_PATH}/{MISC.MODEL_PATH}"
        ...
        self.img_processor = transforms.Normalize(
    ...
    def process_img(self, img):
        ...
        imgTensor = (
            torch.tensor(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), dtype=torch.float32)
            / 255
        )
    ...
    def render(self, params, bg_img=None):
        ...
        if bg_img is None:
            ...
            bg_img = cv2.resize(bg_img, (self.render_size, self.render_size))
    ...
    def render_other_view(self, params, theta=60):
        c = (
            torch.mean(params["v3d_left"], axis=1)
            + torch.mean(params["v3d_right"], axis=1)
        ).unsqueeze(1) / 2
        ...
        img_out, mask_out = self.renderer.render_rgb_orth(
            scale_left=torch.ones((1,)).float().cuda() * 3,
