...
class decoder(nn.Module):
    def __init__(
        ...
        self.gf_layer_left = nn.Sequential(
            *(
                nn.Linear(self.gf_dim, self.gcn_in_dim[0] - 3),
                nn.LayerNorm(self.gcn_in_dim[0] - 3, eps=1e-6),
            )
        )
        self.gf_layer_right = nn.Sequential(
            *(
                nn.Linear(self.gf_dim, self.gcn_in_dim[0] - 3),
                nn.LayerNorm(self.gcn_in_dim[0] - 3, eps=1e-6),
            )
    ...
    def forward(self, x, fmaps):
        ...
        Lf = torch.cat(
            [self.gf_layer_left(x).unsqueeze(1).repeat(1, self.vNum_in, 1), pel], dim=-1
        )
        Rf = torch.cat(
            [self.gf_layer_right(x).unsqueeze(1).repeat(1, self.vNum_in, 1), per],
            dim=-1,
        )
...
def load_decoder(cfg, encoder_info):
    ...
    dense_path = get_dense_color_path(cfg)
    with open(dense_path, "rb") as file:
        dense_coor = pickle.load(file)

    upsample_path = get_upsample_path(cfg)
